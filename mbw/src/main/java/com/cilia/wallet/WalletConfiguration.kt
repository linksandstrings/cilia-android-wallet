...
import com.cilia.wallet.external.partner.model.*
...
class MyceliumNodesResponse(@SerializedName("BTCIL-testnet") val btcilTestnet: BTCNetResponse,
                            @SerializedName("BTCIL-mainnet") val btcilMainnet: BTCNetResponse,
                            @SerializedName("BTC-testnet") val btcTestnet: BTCNetResponse,
...
class WalletConfiguration(private val prefs: SharedPreferences,
        ...
        GlobalScope.launch(Dispatchers.Default, CoroutineStart.DEFAULT) {
            try {
                ...
                if (resp.isSuccessful) {
                    ...
                    val oldelectrumBitcoinIL = electrumBitcoinILServers;
                    ...
                    if (oldelectrumBitcoinIL != electrumBitcoinILServers) {
                        serverElectrumListChangedListener?.serverListBitcoinILChanged(getElectrumBitcoinILEndpoints().toTypedArray())
                    }
    ...
    // Returns the set of electrum servers
    private val electrumBitcoinILServers: Set<String>
        get() = prefs.getStringSet(PREFS_ELECTRUM_BITCOINIL_SERVERS, mutableSetOf(*BuildConfig.ElectrumBitcoinILServers))!!
    ...
    // Returns the list of TcpEndpoint objects
    fun getElectrumBitcoinILEndpoints(): List<TcpEndpoint> {
        val result = ArrayList<TcpEndpoint>()
        electrumBitcoinILServers.forEach {
            try {
                val strs = it.replace(TCP_TLS_PREFIX, "").split(":")
                result.add(TcpEndpoint(strs[0], strs[1].toInt()))
            } catch (ex: Exception) {
    ...
    companion object {
        ...
        const val PREFS_ELECTRUM_BITCOINIL_SERVERS = "electrum_bitcoinil_servers"
