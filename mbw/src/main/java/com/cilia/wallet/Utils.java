...
import com.cilia.wallet.activity.AdditionalBackupWarningActivity;
import com.cilia.wallet.activity.BackupWordListActivity;
import com.cilia.wallet.activity.export.BackupToPdfActivity;
import com.cilia.wallet.activity.export.ExportAsQrActivity;
...
public class Utils {
   ...
   public static void setClipboardString(String string, Context context) {
      try {
         ...
         clipboard.setPrimaryClip(ClipData.newPlainText("Cilia", string));
   ...
   public static void clearClipboardString(Activity activity) {
      try {
         ...
         for (int i = 0; i < 100 ; i++) {
            clipboard.setPrimaryClip(ClipData.newPlainText("Cilia " + i, "wiped " + i));
         }
      } catch (NullPointerException ex) {
...
//      }
   ...
   public static List<WalletAccount<?>> sortAccounts(final Collection<WalletAccount<?>> accounts, final MetadataStorage storage) {
      Ordering<WalletAccount> type = Ordering.natural().onResultOf(new Function<WalletAccount, Integer>() {
         ...
         public Integer apply(@Nullable WalletAccount input) {
            ...
            if (input instanceof HDAccountIL) { // also covers Bip44BCHAccount
               return 0;
            }
            ...
            if (input instanceof SingleAddressAccount) { // also covers SingleAddressBCHAccount
               return checkIsLinked(input, accounts) ? 6 : 2;
            }
   ...
   public static Drawable getDrawableForAccount(AccountViewModel accountView, boolean isSelectedAccount, Resources resources) {
      ...
      //regular HD account
      if (HDAccount.class.isAssignableFrom(accountType) || HDAccountIL.class.isAssignableFrom(accountType)) {
...
//         return "BitcoinIL HD";
   ...
   public static CryptoCurrency getBtcILCoinType() {
      return BuildConfig.FLAVOR.equals("prodnet") ? BitcoinILMain.get() : BitcoinILTest.get();
   }
