...
package com.cilia.wallet.extsig.ledger;
...
import com.mrd.bitillib.SigningRequest;
...
import com.mrd.bitillib.model.ScriptOutput;
import com.mrd.bitillib.model.ScriptOutputP2PKH;
import com.mrd.bitillib.model.ScriptOutputP2SH;
import com.mrd.bitillib.model.ScriptOutputP2WPKH;
...
import com.cilia.wallet.activity.util.AbstractBitcoinILAccountScanManager;
...
import com.mycelium.wapi.wallet.btcil.bip44.ExternalSignatureProviderIL;
import com.mycelium.wapi.wallet.btcil.bip44.ExternalSignaturesAccountConfigIL;
import com.mycelium.wapi.wallet.btcil.bip44.HDAccountContextIL;
import com.mycelium.wapi.wallet.btcil.bip44.HDAccountExternalSignatureIL;
...
public class LedgerManagerIL extends AbstractBitcoinILAccountScanManager implements
        ExternalSignatureProviderIL, OnDiscoveredTagListener {
   ...
   public LedgerManagerIL(Context context, NetworkParameters network, Bus eventBus) {
   ...
   @Override
   public BitcoinILTransaction getSignedTransaction(UnsignedTransaction unsigned,
                                                  HDAccountExternalSignatureIL forAccount) {
      try {
   ...
   private BitcoinILTransaction signInternal(final UnsignedTransaction unsigned, final HDAccountExternalSignatureIL forAccount)
      ...
      class LegacyParams {
         ...
         private LegacyParams(UnsignedTransaction unsigned, HDAccountExternalSignatureIL forAccount) {
      ...
      BitcoinILTransaction unsignedTx = BitcoinILTransaction.fromUnsignedTransaction(unsigned);
      ...
      for (int i = 0; i < inputsNumber; i++) {
         ...
         if (isSegwit) {
            ...
            final BitcoinILOutPoint txOutpoint = currentInput.outPoint;
            ...
            BitcoinILTransaction currentTransaction = BitcoinILTransactionEx.toTransaction(forAccount.getTransaction(currentInput.outPoint.txid));
   ...
   private byte[] signOutput(UnsignedTransaction unsigned, HDAccountExternalSignatureIL forAccount, String commonPath,
      ...
      BitcoinILAddress toSignWith = signingRequest.getPublicKey().toAddress(getNetwork(), derivationType.getAddressType());
   ...
   private String getChangePath(UnsignedTransaction unsigned, HDAccountExternalSignatureIL forAccount, String commonPath) {
      String changePath = "";
      for (TransactionOutput o : unsigned.getOutputs()) {
         BitcoinILAddress toAddress = o.script.getAddress(getNetwork());
   ...
   private String getOutputAddressString(UnsignedTransaction unsigned, HDAccountExternalSignatureIL forAccount) {
      String outputAddress = null;
      for (TransactionOutput output : unsigned.getOutputs()) {
         BitcoinILAddress toAddress = output.script.getAddress(getNetwork());
         Optional<Integer[]> addressId = forAccount.getAddressId(toAddress);
         if (!(addressId.isPresent() && addressId.get()[0] == 1)) {
   ...
   private long calculateTotalSending(UnsignedTransaction unsigned, HDAccountExternalSignatureIL forAccount) {
      long totalSending = 0;
      for (TransactionOutput output : unsigned.getOutputs()) {
         BitcoinILAddress toAddress = output.script.getAddress(getNetwork());
         Optional<Integer[]> addressId = forAccount.getAddressId(toAddress);
   ...
   public boolean upgradeAccount(@NonNull List<? extends HdKeyNode> accountRoots, @NonNull WalletManager walletManager,
      ...
      if (account instanceof HDAccountExternalSignatureIL) {
         // TODO make the module name defined programmatically
         return ((BitcoinILHDModule) walletManager.getModuleById(BitcoinILHDModule.ID)).upgradeExtSigAccount(accountRoots, (HDAccountExternalSignatureIL) account);
      }
      return false;
   }
   ...
   public UUID createOnTheFlyAccount(List<? extends HdKeyNode> accountRoots,
      ...
      if (account == null) {
         account = walletManager.createAccounts(new ExternalSignaturesAccountConfigIL(accountRoots, this, accountIndex)).get(0);
      }
      return account;
   }
   ...
   public int getBIP44AccountType() {
      return HDAccountContextIL.ACCOUNT_TYPE_UNRELATED_X_PUB_EXTERNAL_SIG_LEDGER;
   }
