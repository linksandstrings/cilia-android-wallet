...
package com.cilia.wallet.extsig.common
...
import com.cilia.wallet.Constants.TAG
import com.cilia.wallet.activity.util.AbstractBitcoinILAccountScanManager
import com.cilia.wallet.extsig.common.ExternalSignatureDeviceManagerIL.OnStatusUpdate.CurrentStatus.SHOW_CHANGE_ADDRESS
import com.cilia.wallet.extsig.common.ExternalSignatureDeviceManagerIL.OnStatusUpdate.CurrentStatus.WARNING
import com.mycelium.wapi.wallet.BitcoinILAccountScanManager
import com.mycelium.wapi.wallet.btcil.bip44.*
import com.satoshilabs.trezor.lib.protobuf.TrezorType.RequestType.TXOUTPUT

abstract class ExternalSignatureDeviceManagerIL(context: Context, network: NetworkParameters, eventBus: Bus) : AbstractBitcoinILAccountScanManager(context, network, eventBus), ExternalSignatureProviderIL {
    ...
    private fun initialize(): Boolean {
        ...
        while (!signatureDevice.isDevicePluggedIn) {
            try {
                setState(BitcoinILAccountScanManager.Status.unableToScan, currentAccountState)
                Thread.sleep(4000)
    ...
    private fun getChangeAddress(unsigned: UnsignedTransaction, forAccount: HDAccountExternalSignatureIL) =
    ...
    override fun getSignedTransaction(unsigned: UnsignedTransaction, forAccount: HDAccountExternalSignatureIL): BitcoinILTransaction? {
        ...
        setState(BitcoinILAccountScanManager.Status.readyToScan, currentAccountState)
        ...
        while (true) {
            ...
            val requestedTx: BitcoinILTransaction
            requestedTx = if (txRequestDetailsType.hasTxHash()) {
                ...
                val requestHash = BitcoinILSha256Hash.of(txRequestDetailsType.txHash.toByteArray())
                BitcoinILTransactionEx.toTransaction(forAccount.getTransaction(requestHash))
            } else {
                // trezor requested information about the to-be-signed tx
                BitcoinILTransaction.fromUnsignedTransaction(unsigned)
            }
        ...
        return try {
            BitcoinILTransaction.fromByteReader(ByteReader(signedTx.toBytes()))
            // TODO: 13.10.18 add this check back in and make it work with segwit.
            //checkSignedTransaction(unsigned, signedTx);
        } catch (e: BitcoinILTransaction.BitcoinILTransactionParsingException) {
    ...
    override fun upgradeAccount(accountRoots: List<HdKeyNode>, walletManager: WalletManager,
        ...
        return if (account is HDAccountExternalSignatureIL) {
            // TODO make the module name defined programmatically
            return (walletManager.getModuleById(BitcoinILHDModule.ID) as BitcoinILHDModule).upgradeExtSigAccount(accountRoots, account)
        } else {
            false
        }
    }
    ...
    override fun createOnTheFlyAccount(accountRoots: List<HdKeyNode>, walletManager: WalletManager, accountIndex: Int) =
            accountRoots.firstOrNull { walletManager.hasAccount(it.uuid) }?.uuid
                    ?: walletManager.createAccounts(ExternalSignaturesAccountConfigIL(
    ...
    private fun filterMessages(msg: Message?, transaction: UnsignedTransaction? = null, forAccount: HDAccountIL? = null): Message? {
        return when (msg) {
    ...
    private fun processPassphraseStateRequest(transaction: UnsignedTransaction?, account: HDAccountIL?): Message? {
    ...
    private fun processPassphraseRequest(onDevice: Boolean, transaction: UnsignedTransaction?, account: HDAccountIL?): Message? {
    ...
    private fun processPinMatrixRequest(transaction: UnsignedTransaction?, account: HDAccountIL?): Message? {
    ...
    private fun processButtonRequest(message: Message, transaction: UnsignedTransaction?, account: HDAccountIL?): Message? {
 ...
 */
fun showChange(unsigned: UnsignedTransaction, network: NetworkParameters, account: HDAccountIL): Boolean {
