...
import com.mycelium.wapi.wallet.BitcoinILAccountScanManager
...
abstract class AbstractBitcoinILAccountScanManager @JvmOverloads constructor(protected val context: Context,
                                                                             protected val network: NetworkParameters,
                                                                             protected val eventBus: Bus,
                                                                             private val coinType: CryptoCurrency = Utils.getBtcILCoinType()) : BitcoinILAccountScanManager {

    private var scanAsyncTask: AsyncTask<Void, ScanStatus, Void>? = null
    private val foundAccounts = ArrayList<BitcoinILAccountScanManager.HdKeyNodeWrapper>()
    ...
    @Volatile
    var currentAccountState: BitcoinILAccountScanManager.AccountStatus = BitcoinILAccountScanManager.AccountStatus.unknown
    @Volatile
    var currentState: BitcoinILAccountScanManager.Status = BitcoinILAccountScanManager.Status.unableToScan
    @Volatile
    private var nextUnusedAccounts = listOf<HdKeyNode>()

    open inner class ScanStatus(val state: BitcoinILAccountScanManager.Status, val accountState: BitcoinILAccountScanManager.AccountStatus)

    inner class FoundAccountStatus(val account: BitcoinILAccountScanManager.HdKeyNodeWrapper) : ScanStatus(BitcoinILAccountScanManager.Status.readyToScan, BitcoinILAccountScanManager.AccountStatus.scanning)

    protected abstract fun onBeforeScan(): Boolean

    override fun startBackgroundAccountScan(scanningCallback: BitcoinILAccountScanManager.AccountCallback) {
        if (currentAccountState == BitcoinILAccountScanManager.AccountStatus.scanning || currentAccountState == BitcoinILAccountScanManager.AccountStatus.done) {
            // currently scanning or have already all account - just post the events for all already known accounts
            for (a in foundAccounts) {
                eventBus.post(BitcoinILAccountScanManager.OnAccountFound(a))
            }
        } else {
            ...
            scanAsyncTask = object : AsyncTask<Void, ScanStatus, Void>() {
                override fun doInBackground(vararg voids: Void): Void? {
                    publishProgress(ScanStatus(BitcoinILAccountScanManager.Status.initializing, BitcoinILAccountScanManager.AccountStatus.unknown))
                    if (onBeforeScan()) {
                        publishProgress(ScanStatus(BitcoinILAccountScanManager.Status.readyToScan, BitcoinILAccountScanManager.AccountStatus.scanning))
                    } else {
                        return null
                    }
                    ...
                    do {
                        ...
                        val acc = BitcoinILAccountScanManager.HdKeyNodeWrapper(accountPathsToScan.values, rootNodes, null)
                        ...
                        wasUsedOrAccountsLookahead = if (newAccount != null) {
                            ...
                            val foundAccount = BitcoinILAccountScanManager.HdKeyNodeWrapper(accountPathsToScan.values, rootNodes, newAccount)
                    ...
                    publishProgress(ScanStatus(BitcoinILAccountScanManager.Status.readyToScan, BitcoinILAccountScanManager.AccountStatus.done))
                    return null
                }
                ...
                override fun onPreExecute() {
                    super.onPreExecute()
                    currentState = BitcoinILAccountScanManager.Status.initializing
                }
                ...
                override fun onProgressUpdate(vararg stateInfo: ScanStatus) {
                    for (si in stateInfo) {
                        ...
                        if (si is FoundAccountStatus) {
                            ...
                            eventBus.post(BitcoinILAccountScanManager.OnAccountFound(foundAccount))
    ...
    @Synchronized
    protected fun setState(state: BitcoinILAccountScanManager.Status, accountState: BitcoinILAccountScanManager.AccountStatus) {
        mainThreadHandler.post { eventBus.post(BitcoinILAccountScanManager.OnStatusChanged(state, accountState)) }
        currentState = state
    ...
    override fun stopBackgroundAccountScan() {
        if (scanAsyncTask != null) {
            ...
            currentAccountState = BitcoinILAccountScanManager.AccountStatus.unknown
        }
    }
    ...
    override fun forgetAccounts() {
        if (currentAccountState == BitcoinILAccountScanManager.AccountStatus.scanning) {
    ...
    protected fun waitForPassphrase(): Optional<String> {
        ...
        mainThreadHandler.post { eventBus.post(BitcoinILAccountScanManager.OnPassphraseRequest()) }
    ...
    protected fun postErrorMessage(msg: String): Boolean {
        mainThreadHandler.post { eventBus.post(BitcoinILAccountScanManager.OnScanError(msg)) }
        return true
    }
    ...
    protected fun postErrorMessage(msg: String, failureType: TrezorType.FailureType): Boolean {
        mainThreadHandler.post {
            ...
            if (failureType == TrezorType.FailureType.Failure_NotInitialized) {
                eventBus.post(BitcoinILAccountScanManager.OnScanError(msg, BitcoinILAccountScanManager.OnScanError.ErrorType.NOT_INITIALIZED))
            } else {
                eventBus.post(BitcoinILAccountScanManager.OnScanError(msg))
            }
        }
        return true
    }
    ...
    override fun getAccountPathsToScan(lastPath: HdKeyPath?, wasUsedOrAccountsLookahead: Boolean, coinType: CryptoCurrency): Map<BipDerivationType, HdKeyPath> {
        ...
        if (lastPath == null) {
            return if (coinType == Utils.getBtcILCoinType()) {
        ...
        return if (wasUsedOrAccountsLookahead) {
            if (coinType == Utils.getBtcILCoinType()) {
    ...
    companion object {
        ...
        val BIP44COIN_TYPE = HdKeyPath.BIP44.getCoinTypeBitcoin(BuildConfig.FLAVOR == "btciltestnet")
        val BIP49COIN_TYPE = HdKeyPath.BIP49.getCoinTypeBitcoin(BuildConfig.FLAVOR == "btciltestnet")
        val BIP84COIN_TYPE = HdKeyPath.BIP84.getCoinTypeBitcoin(BuildConfig.FLAVOR == "btciltestnet")
    }
}
