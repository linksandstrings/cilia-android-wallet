...
import com.cilia.wallet.activity.modern.AddressBookFragment
...
import com.cilia.wallet.activity.send.VerifyPaymentRequestActivity
import com.cilia.wallet.activity.util.*
...
abstract class SendCoinsViewModel(application: Application) : AndroidViewModel(application) {
    ...
    fun updateClipboardUri() {
        ...
        model.clipboardUri.value = if (uriPattern.matcher(string).matches()) {
            ...
            if (address != null) {
                if (address is BtcILAddress) {
                    AssetBitcoinILUriParser.createUriByCoinType(model.account.coinType, address, null, null, null)
                } else {
    ...
    private fun isUriMatchAccountCoinType(uri: AssetUri, coinType: CryptoCurrency): Boolean {
        return when (uri) {
            ...
            is BitcoinILUri -> coinType == Utils.getBtcILCoinType()
    ...
    private fun handleScanResults(resultCode: Int, data: Intent?, activity: Activity) {
        if (resultCode != Activity.RESULT_OK) {
            ...
            when (data?.getSerializableExtra(StringHandlerActivity.RESULT_TYPE_KEY) as ResultType) {
                ...
                ResultType.ADDRESS -> {
                    var currentAddress = data.getAddress();
                    var currentCoinType = currentAddress.coinType
                    var accountCoinType = getAccount().basedOnCoinType
