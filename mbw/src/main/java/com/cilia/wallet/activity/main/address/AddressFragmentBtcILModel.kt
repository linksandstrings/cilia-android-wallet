...
class AddressFragmentBtcILModel(val app: Application) : AddressFragmentViewModel(app) {
    ...
    override fun qrClickReaction(activity: FragmentActivity) {
        ...
        (model.account as AbstractBtcILAccount).setDefaultAddressType(currentType)
    ...
    private fun getNextType(): AddressType {
        val addressTypes = (model.account as AbstractBtcILAccount).availableAddressTypes
