...
import com.cilia.wallet.activity.util.GlobalBlockExplorerManager;
import com.cilia.wallet.activity.util.Pin;
import com.cilia.wallet.activity.util.ValueExtensionsKt;
import com.cilia.wallet.api.AndroidAsyncApi;
import com.cilia.wallet.bitid.ExternalService;
import com.cilia.wallet.colu.SqliteColuManagerBacking;
import com.cilia.wallet.event.AccountChanged;
import com.cilia.wallet.event.BalanceChanged;
import com.cilia.wallet.event.EventTranslator;
...
import com.cilia.wallet.event.SyncStarted;
import com.cilia.wallet.event.SyncStopped;
import com.cilia.wallet.event.TorStateChanged;
import com.cilia.wallet.event.TransactionBroadcasted;
import com.cilia.wallet.extsig.common.ExternalSignatureDeviceManager;
import com.cilia.wallet.extsig.common.ExternalSignatureDeviceManagerIL;
import com.cilia.wallet.extsig.keepkey.KeepKeyManager;
import com.cilia.wallet.extsig.keepkey.KeepKeyManagerIL;
import com.cilia.wallet.extsig.ledger.LedgerManager;
import com.cilia.wallet.extsig.ledger.LedgerManagerIL;
import com.cilia.wallet.extsig.trezor.TrezorManager;
import com.cilia.wallet.extsig.trezor.TrezorManagerIL;
import com.cilia.wallet.fio.AbiFioSerializationProviderWrapper;
import com.cilia.wallet.lt.LocalTraderManager;
import com.cilia.wallet.persistence.MetadataStorage;
import com.cilia.wallet.persistence.TradeSessionDb;
import com.cilia.wallet.wapi.SqliteBtcILWalletManagerBacking;
import com.cilia.wallet.wapi.SqliteBtcWalletManagerBacking;
...
import com.mycelium.wapi.content.btcil.BitcoinILUriParser;
...
import com.mycelium.wapi.wallet.btcil.BTCSettingsIL;
import com.mycelium.wapi.wallet.btcil.BtcILAddress;
import com.mycelium.wapi.wallet.btcil.BtcILWalletManagerBacking;
import com.mycelium.wapi.wallet.btcil.ChangeAddressModeIL;
import com.mycelium.wapi.wallet.btcil.InMemoryBtcILWalletManagerBacking;
...
import com.mycelium.wapi.wallet.btcil.bip44.ExternalSignatureProviderProxyIL;
import com.mycelium.wapi.wallet.btcil.bip44.HDAccountIL;
import com.mycelium.wapi.wallet.btcil.bip44.HDAccountContextIL;
import com.mycelium.wapi.wallet.btcil.single.AddressSingleConfigIL;
import com.mycelium.wapi.wallet.btcil.single.BitcoinILSingleAddressModule;

import com.mycelium.wapi.wallet.btcil.single.PrivateSingleConfigIL;
import com.mycelium.wapi.wallet.btcil.single.PublicPrivateKeyStoreIL;
...
public class MbwManager {
    ...
    private final ExternalSignatureDeviceManagerIL _trezorManagerIL;
    private final KeepKeyManager _keepkeyManager;
    private final KeepKeyManagerIL _keepkeyManagerIL;
    private final LedgerManager _ledgerManager;
    private final LedgerManagerIL _ledgerManagerIL;
    ...
    private static final com.mrd.bitlib.model.AddressType defaultBitcoinILAddressType = com.mrd.bitlib.model.AddressType.P2SH_P2WPKH;
    private ChangeAddressMode changeAddressMode;
    private ChangeAddressModeIL changeAddressModeIL;
    ...
    private MbwManager(Context evilContext) {
        ...
        changeAddressModeIL = ChangeAddressModeIL.valueOf(preferences.getString(Constants.CHANGE_ADDRESS_MODE,
                ChangeAddressMode.PRIVACY.name()));
        ...
        _trezorManagerIL = new TrezorManagerIL(_applicationContext, getBitcoinILNetwork(), getEventBus());
        _keepkeyManager = new KeepKeyManager(_applicationContext, getNetwork(), getEventBus());
        _keepkeyManagerIL = new KeepKeyManagerIL(_applicationContext, getBitcoinILNetwork(), getEventBus());
        _ledgerManager = new LedgerManager(_applicationContext, getNetwork(), getEventBus());
        _ledgerManagerIL = new LedgerManagerIL(_applicationContext, getBitcoinILNetwork(), getEventBus());
    ...
    private ContentResolver createContentResolver() {
        ContentResolver result = new ContentResolver();
        result.add(new BitcoinILUriParser(getBitcoinILNetwork()));
        result.add(new BitcoinUriParser(getNetwork()));
//        result.add(new MTUriParser(getNetwork()));
//        result.add(new MSSUriParser(getNetwork()));
//        result.add(new RMCUriParser(getNetwork()));
//        result.add(new EthUriParser(getNetwork(), getSupportedERC20Tokens()));
//        result.add(new FIOUriParser(getNetwork()));
        return result;
    }
    ...
    private void initBTCILSettings() {
        BTCSettingsIL btcSettingsIL = new BTCSettingsIL(defaultBitcoinILAddressType, new ReferenceIL<>(changeAddressModeIL));
        currenciesSettingsMap.put(BitcoinILHDModule.ID, btcSettingsIL);
        currenciesSettingsMap.put(BitcoinILSingleAddressModule.ID, btcSettingsIL);
    }
    ...
    private WapiClientElectrumX initWapi() {
        ...
        List<TcpEndpoint> bitcoinILtcpEndpoints = configuration.getElectrumBitcoinILEndpoints();
        ...
        WapiClientElectrumX wapiClientElectrumX = new WapiClientElectrumX(new ServerEndpoints(wapiEndpoints.toArray(new HttpEndpoint[0])),
                tcpEndpoints.toArray(new TcpEndpoint[0]), bitcoinILtcpEndpoints.toArray(new TcpEndpoint[0]), version, Build.VERSION.SDK_INT);
    ...
    public ChangeAddressModeIL getChangeAddressModeIL() {
        return changeAddressModeIL;
    }
    ...
    public void setChangeAddressModeIL(ChangeAddressModeIL changeAddressModeIL) {
        this.changeAddressModeIL = changeAddressModeIL;
        BTCSettingsIL currencySettings = (BTCSettingsIL) _walletManager.getCurrencySettings(BitcoinILHDModule.ID);
        if (currencySettings != null) {
            currencySettings.setChangeAddressMode(changeAddressModeIL);
            _walletManager.setCurrencySettings(BitcoinILHDModule.ID, currencySettings);
            getEditor().putString(Constants.CHANGE_ADDRESS_MODE, changeAddressModeIL.toString()).apply();
        }
    }
     ...
     * @param environment the Cilia environment
    ...
    private WalletManager createWalletManager(final Context context, final MbwEnvironment environment,
        ...
        BtcILWalletManagerBacking backingIL = new SqliteBtcILWalletManagerBacking(context);
        ...
        ExternalSignatureProviderProxyIL externalSignatureProviderProxyIL = new ExternalSignatureProviderProxyIL(
                getTrezorManagerIL(),
                getKeepKeyManagerIL(),
                getLedgerManagerIL()
        );
        ...
        com.mrd.bitillib.model.NetworkParameters bitcoinILnetworkParameters = environment.getBitcoinILNetwork();
        PublicPrivateKeyStore publicPrivateKeyStore = new PublicPrivateKeyStore(secureKeyValueStore);
        PublicPrivateKeyStoreIL publicPrivateKeyStoreIL = new PublicPrivateKeyStoreIL(secureKeyValueStore);
...
//        };
        ...
        AccountEventManagerIL accountEventManagerIL = new AccountEventManagerIL(walletManager);
        walletManager.add(new BitcoinILHDModule(backing, secureKeyValueStore, bitcoinILnetworkParameters, _wapi, (BTCSettingsIL) currenciesSettingsMap.get(BitcoinILHDModule.ID), getMetadataStorage(),
                externalSignatureProviderProxyIL, migrationProgressTracker, accountEventManagerIL));

        BitcoinILSingleAddressModule saModuleIL = new BitcoinILSingleAddressModule(backingIL, publicPrivateKeyStoreIL,
                bitcoinILnetworkParameters, _wapi, (BTCSettingsIL) currenciesSettingsMap.get(BitcoinILSingleAddressModule.ID), walletManager, getMetadataStorage(),
                migrationProgressTracker, accountEventManagerIL);
        walletManager.add(saModuleIL);
...
//        walletManager.add(new InvestmentModule(getMetadataStorage()));
    ...
    private class AccountEventManagerIL implements AbstractBtcILAccount.EventHandler {
        private WalletManager walletManager;

        AccountEventManagerIL(WalletManager walletManager) {
            this.walletManager = walletManager;
        }
    ...
    private WalletManager createTempWalletManager(MbwEnvironment environment) {
        ...
        BtcILWalletManagerBacking backingIL = new InMemoryBtcILWalletManagerBacking();
        ...
        AccountEventManagerIL accountEventManagerIL = new AccountEventManagerIL(walletManager);
        walletManager.add(new BitcoinILHDModule(backing, secureKeyValueStore, bitcoinILnetworkParameters, _wapi, (BTCSettingsIL) currenciesSettingsMap.get(BitcoinILHDModule.ID), getMetadataStorage(),
                null, null, accountEventManagerIL));
        walletManager.add(new BitcoinILSingleAddressModule(backingIL, publicPrivateKeyStoreIL,
                bitcoinILnetworkParameters, _wapi, (BTCSettingsIL) currenciesSettingsMap.get(BitcoinILSingleAddressModule.ID), walletManager, getMetadataStorage(),
                null, accountEventManagerIL));
...
//
    ...
    public com.mrd.bitillib.model.NetworkParameters getBitcoinILNetwork() {
        return _environment.getBitcoinILNetwork();
    }
    ...
    public UUID createOnTheFlyAccount(Address address) {
        ...
        } else if (address instanceof BtcILAddress) {
            accountId = _tempWalletManager.createAccounts(new AddressSingleConfigIL(
                    new BtcILAddress(Utils.getBtcILCoinType(), ((BtcILAddress) address).getAddress()))).get(0);
    ...
    public UUID createOnTheFlyBtcilAccount(com.mrd.bitillib.crypto.InMemoryPrivateKey privateKey) {
        UUID accountId;
        accountId = _tempWalletManager.createAccounts(new PrivateSingleConfigIL(privateKey, AesKeyCipher.defaultKeyCipher())).get(0);
        _tempWalletManager.getAccount(accountId).setAllowZeroConfSpending(true);
        _tempWalletManager.startSynchronization(accountId);
        return accountId;
    }
    ...
    public InMemoryPrivateKey obtainPrivateKeyForAccount(WalletAccount account, String website, KeyCipher cipher) {
        ...
        } else if (account instanceof SingleAddressAccountIL) {
            // For single address accounts we use the private key directly
            try {
                return account.getPrivateKey(cipher);
            } catch (KeyCipher.InvalidKeyCipher invalidKeyCipher) {
        ...
        } else if (account instanceof HDAccountIL && ((HDAccountIL) account).getAccountType() == HDAccountContextIL.ACCOUNT_TYPE_FROM_MASTERSEED) {
            // For BIP44 accounts we derive a private key from the BIP32 hierarchy
            try {
                Bip39.MasterSeed masterSeed = masterSeedManager.getMasterSeed(cipher);
                int accountIndex = ((HDAccountIL) account).getAccountIndex();
                return createBip32WebsitePrivateKey(masterSeed.getBip32Seed(), accountIndex, website);
            } catch (KeyCipher.InvalidKeyCipher invalidKeyCipher) {
                throw new RuntimeException(invalidKeyCipher);
            }
    ...
    public UUID createAdditionalBip44AccountIL(Context context) {
        UUID accountId = _walletManager.createAccounts(new AdditionalHDAccountConfigIL()).get(0);
        //set default label for the created BitcoinIL HD account
        WalletAccount account = _walletManager.getAccount(accountId);
        String defaultName = Utils.getNameForNewAccount(account, context);
        MetadataStorage.INSTANCE.storeAccountLabel(accountId, defaultName);
        return accountId;
    ...
    public ExternalSignatureDeviceManagerIL getTrezorManagerIL() {
        return _trezorManagerIL;
    }

    public KeepKeyManager getKeepKeyManager() {
        return _keepkeyManager;
    }
    public KeepKeyManagerIL getKeepKeyManagerIL() {
        return _keepkeyManagerIL;
    }

    public LedgerManager getLedgerManager() {
        return _ledgerManager;
    }
    public LedgerManagerIL getLedgerManagerIL() {
        return _ledgerManagerIL;
    }
