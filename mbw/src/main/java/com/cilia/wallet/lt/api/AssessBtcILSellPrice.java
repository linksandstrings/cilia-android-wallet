...
public class AssessBtcILSellPrice extends Request {
   private static final long serialVersionUID = 1L;

   public final BtcSellPriceParameters params;

   public AssessBtcILSellPrice(BtcSellPriceParameters params) {
      super(true, false);
      this.params = params;
   }
         ...
         synchronized (subscribers) {
            for (final LocalTraderEventSubscriber s : subscribers) {
               s.getHandler().post(new Runnable() {
                  ...
                  @Override
                  public void run() {
                     s.onLtBtcILSellPriceAssesed(btcSellPrice, AssessBtcILSellPrice.this);
                  }
               });
            }
         }
