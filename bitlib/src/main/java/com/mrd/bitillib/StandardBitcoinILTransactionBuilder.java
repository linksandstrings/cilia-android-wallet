...
public class StandardBitcoinILTransactionBuilder {
   // hash size 32 + output index size 4 + script length 1 + max. script size for compressed keys 107 + sequence number 4
   // also see https://github.com/bitcoinil/bitcoinil/blob/master/src/primitives/transaction.h#L190
   public static final int MAX_INPUT_SIZE = 32 + 4 + 1 + 107 + 4;
   private NetworkParameters _network;
   private List<TransactionOutput> _outputs;

   public static class InsufficientBTCILException extends Exception {
      ...
      public InsufficientBTCILException(long sending, long fee) {
   ...
   public static class BTCILOutputTooSmallException extends Exception {
      //todo consider refactoring this into a composite return value instead of an exception. it is not really "exceptional"
      private static final long serialVersionUID = 1L;

      public long value;

      public BTCILOutputTooSmallException(long value) {
         super("An output was added with a value of " + value
             + " satoshis, which is smaller than the minimum accepted by the BitcoinIL network");
      }
   }
   ...
   public StandardBitcoinILTransactionBuilder(NetworkParameters network) {
      _network = network;
      _outputs = new LinkedList<>();
   }

   public void addOutput(BitcoinILAddress sendTo, long value) throws BTCILOutputTooSmallException {
      addOutput(createOutput(sendTo, value, _network));
   }

   public void addOutput(TransactionOutput output) throws BTCILOutputTooSmallException {
      if (output.value < MINIMUM_OUTPUT_VALUE) {
         throw new BTCILOutputTooSmallException(output.value);
      }
      _outputs.add(output);
   }

   public void addOutputs(OutputList outputs) throws BTCILOutputTooSmallException {
   ...
   public static TransactionOutput createOutput(BitcoinILAddress sendTo, long value, NetworkParameters network) {
      ScriptOutput script;
   ...
   public static List<byte[]> generateSignatures(SigningRequest[] requests, IPrivateKeyRing keyRing) {
      ...
      for (SigningRequest request : requests) {
         BitcoinILSigner signer = keyRing.findSignerByPublicKey(request.getPublicKey());
         if (signer == null) {
            // This should not happen as we only work on outputs that we have
            // keys for
            throw new RuntimeException("Private key not found");
         }
         byte[] signature = signer.makeStandardBitcoinILSignature(request.getToSign());
   ...
   public UnsignedTransaction createUnsignedTransaction(Collection<UnspentTransactionOutput> inventory,
                                                        @Nonnull BitcoinILAddress changeAddress, IPublicKeyRing keyRing,
                                                        NetworkParameters network, long minerFeeToUse)
       throws InsufficientBTCILException, UnableToBuildTransactionException {
      ...
      if (estimatedFeePerKb > BitcoinILTransaction.MAX_MINER_FEE_PER_KB) {
   ...
   @VisibleForTesting
   BitcoinILAddress getRichest(Collection<UnspentTransactionOutput> unspent, final NetworkParameters network) {
      Preconditions.checkArgument(!unspent.isEmpty());
      Function<UnspentTransactionOutput, BitcoinILAddress> txout2Address = new Function<UnspentTransactionOutput, BitcoinILAddress>() {
         @Override
         public BitcoinILAddress apply(UnspentTransactionOutput input) {
            return input.script.getAddress(network);
         }
      };
      Multimap<BitcoinILAddress, UnspentTransactionOutput> index = Multimaps.index(unspent, txout2Address);
      BitcoinILAddress ret = getRichest(index);
      return Preconditions.checkNotNull(ret);
   }

   private BitcoinILAddress getRichest(Multimap<BitcoinILAddress, UnspentTransactionOutput> index) {
      BitcoinILAddress ret = null;
      long maxSum = 0;
      for (BitcoinILAddress address : index.keys()) {
   ...
   public static BitcoinILTransaction finalizeTransaction(UnsignedTransaction unsigned, List<byte[]> signatures) {
      ...
      return new BitcoinILTransaction(1, inputs, unsigned.getOutputs(), unsigned.getLockTime());
   }
   ...
   private class FifoCoinSelector implements CoinSelector {
      ...
      FifoCoinSelector(long feeSatPerKb, List<UnspentTransactionOutput> unspent, AddressType changeType)
          throws InsufficientBTCILException {
         ...
         while (foundSat < feeSat + outputSum) {
            ...
            if (unspentTransactionOutput == null) {
               ...
               throw new InsufficientBTCILException(outputSum, feeSat);
            }
            ...
            if (needChangeOutputInEstimation(allFunding, outputSum, feeSatPerKb)) {
               estimatorBuilder.addOutput(BitcoinILAddress.getNullAddress(_network, changeType).getType());
            }
