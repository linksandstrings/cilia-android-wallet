...
import com.mrd.bitillib.crypto.ec.EcTools;
...
import com.mrd.bitillib.crypto.PrivateKey;
...
/**
 * A BitcoinIL private key that is kept in memory.
 */
public class InMemoryPrivateKey extends PrivateKey implements KeyExporter, Serializable {
   ...
   public InMemoryPrivateKey(BitcoinILSha256Hash hash, boolean compressed) {
   ...
   public static Optional<InMemoryPrivateKey> fromBase58MiniFormat(String base58, NetworkParameters network) {
      ...
      BitcoinILSha256Hash privateKeyBytes = HashUtils.sha256(base58.getBytes());
      try {
   ...
   private static class DsaSignatureNonceGenDeterministic extends DsaSignatureNonceGen {
      ...
      private final BitcoinILSha256Hash messageHash;
      private final KeyExporter privateKey;

      private DsaSignatureNonceGenDeterministic(BitcoinILSha256Hash messageHash, KeyExporter privateKey) {
   ...
   @Override
   @NotNull
   protected Signature generateSignature(@NotNull BitcoinILSha256Hash messageHash) {
      return generateSignatureInternal(messageHash, new DsaSignatureNonceGenDeterministic(messageHash, this));
   }

   @Override
   @NotNull
   protected Signature generateSignature(@NotNull BitcoinILSha256Hash messageHash, @NotNull RandomSource randomSource) {
      return generateSignatureInternal(messageHash, new DsaSignatureNonceGenRandom(randomSource));
   }

   private Signature generateSignatureInternal(BitcoinILSha256Hash messageHash, DsaSignatureNonceGen kGen) {
