...
import com.mrd.bitillib.crypto.ec.Parameters
import com.mrd.bitillib.crypto.ec.Point
import com.mrd.bitillib.model.*
import com.mrd.bitillib.util.*
...
class PublicKey(val publicKeyBytes: ByteArray) : Serializable {
     ...
     */
    @JvmOverloads
    fun toAddress(networkParameters: NetworkParameters, addressType: AddressType, ignoreCompression: Boolean = false): BitcoinILAddress {
     ...
     */
    private fun toNestedP2WPKH(networkParameters: NetworkParameters, ignoreCompression: Boolean = false): BitcoinILAddress {
        if (ignoreCompression || isCompressed) {
            ...
            return BitcoinILAddress.fromP2SHBytes(HashUtils.addressHash(
     ...
     */
    private fun toP2PKHAddress(networkParameters: NetworkParameters): BitcoinILAddress =
            BitcoinILAddress.fromStandardBytes(publicKeyHash, networkParameters)
    ...
    fun verifyStandardBitcoinILSignature(data: BitcoinILSha256Hash, signature: ByteArray, forceLowS: Boolean): Boolean {
    ...
    // same as verifyStandardBitcoinILSignature, but don't enforce the hash-type check
    fun verifyDerEncodedSignature(data: BitcoinILSha256Hash, signature: ByteArray): Boolean {
        // Decode parameters r and s
        val reader = ByteReader(signature)
        val params = Signatures.decodeSignatureParameters(reader) ?: return false
    ...
    companion object {
        ...
        private fun SUPPORTED_ADDRESS_TYPES(isCompressed: Boolean) = if (isCompressed) {
            ...
            // [BIP143](https://github.com/bitcoinil/bips/blob/master/bip-0143.mediawiki#restrictions-on-public-key-type).
