...
import com.mrd.bitillib.crypto.ec.Curve;
...
public class SignedMessage implements Serializable {
   ...
   public static SignedMessage validate(BitcoinILAddress address, String message, String signatureBase64)
   ...
   public static void validateAddressMatches(BitcoinILAddress address, PublicKey key) throws WrongSignatureException {
      Collection<BitcoinILAddress> possibleAddresses = key.getAllSupportedAddresses(address.getNetwork()).values();
   ...
   private static RecoveryInfo recoverFromSignature(String message, byte[] signatureEncoded, Signature sig)
      ...
      BitcoinILSha256Hash messageHash = HashUtils.doubleSha256(messageBytes);
    ...
    */
   public static PublicKey recoverFromSignature(int recId, Signature sig, BitcoinILSha256Hash message, boolean compressed) {
   ...
   public byte[] bitcoinilEncodingOfSignature() {
      if (recId == -1)
   ...
   public String getBase64Signature() {
      return Base64.encodeToString(bitcoinilEncodingOfSignature(), false);
   }

   public byte[] getDerEncodedSignature(){
      //byte[] rsValues = bitcoinilEncodingOfSignature();
