...
public class Bip38 {
   ...
   public static String encryptNoEcMultiply(String passphrase, String base58EncodedPrivateKey) throws InterruptedException {
      ...
      BitcoinILAddress address = key.getPublicKey().toAddress(NetworkParameters.productionNetwork, AddressType.P2PKH);
   ...
   public static String encryptNoEcMultiply(byte[] stretcedKeyMaterial, InMemoryPrivateKey key, byte[] salt) {
      ...
      // Checksum
      BitcoinILSha256Hash checkSum = HashUtils.doubleSha256(encoded, 0, 39);
   ...
   public static String decryptEcMultiply(Bip38PrivateKey bip38Key, String passphrase,
      ...
      // Validate result
      BitcoinILAddress address = finalKey.getPublicKey().toAddress(network, AddressType.P2PKH);
   ...
   public static String decryptNoEcMultiply(Bip38PrivateKey bip38Key, byte[] stretcedKeyMaterial,
      ...
      for (AddressType type : AddressType.values()) {
         BitcoinILAddress address = key.getPublicKey().toAddress(network, type);
         byte[] newSalt = calculateScryptSalt(address);
         if (BitUtils.areEqual(bip38Key.salt, newSalt)) {
            // Get SIPA format
   ...
   /**
    * Calculate scrypt salt from BitcoinIL address
    * <p/>
    * BIP38 uses a scrypt salt which depends on the BitcoinIL address. This method
    * takes a BitcoinIL address and calculates the BIP38 salt.
    */
   public static byte[] calculateScryptSalt(BitcoinILAddress address) {
      BitcoinILSha256Hash hash = HashUtils.doubleSha256(address.toString().getBytes());
