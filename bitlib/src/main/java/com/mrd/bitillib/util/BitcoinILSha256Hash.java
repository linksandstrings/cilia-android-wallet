 ...
 * factory methods.
 */
public class BitcoinILSha256Hash implements Serializable, Comparable<BitcoinILSha256Hash> {
   private static final long serialVersionUID = 1L;

   public static final int HASH_LENGTH = 32;
   public static final BitcoinILSha256Hash ZERO_HASH = of(new byte[HASH_LENGTH]);

   final private byte[] _bytes;
   private int _hash;

   public BitcoinILSha256Hash(byte[] bytes) {
      Preconditions.checkArgument(bytes.length == HASH_LENGTH);
      this._bytes = bytes;
      _hash = -1;
   }

   public static BitcoinILSha256Hash fromString(String hexString) {
      try {
         byte[] b = HexUtils.toBytes(hexString);
         if (b.length != HASH_LENGTH) {
            return null;
         }
         return new BitcoinILSha256Hash(b);
    ...
    *           to be stored
    */
   public static BitcoinILSha256Hash of(byte[] bytes) {
      return new BitcoinILSha256Hash(bytes);
   }

   public static BitcoinILSha256Hash copyOf(byte[] bytes, int offset) {
      return new BitcoinILSha256Hash(bytes, offset);
   }

   private BitcoinILSha256Hash(byte[] bytes, int offset) {
   ...
   public boolean equals(Object other) {
      ...
      if (!(other instanceof BitcoinILSha256Hash))
         return false;
      return Arrays.equals(_bytes, ((BitcoinILSha256Hash) other)._bytes);
   }
   ...
   @Override
   public int compareTo(BitcoinILSha256Hash o) {
   ...
   public BitcoinILSha256Hash reverse() {
      return new BitcoinILSha256Hash(BitUtils.reverseBytes(_bytes));
   }
