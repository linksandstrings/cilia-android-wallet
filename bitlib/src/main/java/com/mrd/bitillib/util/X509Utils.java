...
public class X509Utils {
    ...
    /** The string that prefixes all text messages signed using BitcoinIL keys. */
    private static final String BITCOINIL_SIGNED_MESSAGE_HEADER = "BitcoinIL Signed Message:\n";
    private static final byte[] BITCOINIL_SIGNED_MESSAGE_HEADER_BYTES = BITCOINIL_SIGNED_MESSAGE_HEADER.getBytes(StandardCharsets.UTF_8);

    /**
     * <p>Given a textual message, returns a byte buffer formatted as follows:</p>
     * <p>{@code [24] "BitcoinIL Signed Message:\n" [message.length as a varint] message}</p>
     */

    public static byte[] formatMessageForSigning(String message) {
        try {
            ByteArrayOutputStream bos = new ByteArrayOutputStream();
            bos.write(BITCOINIL_SIGNED_MESSAGE_HEADER_BYTES.length);
            bos.write(BITCOINIL_SIGNED_MESSAGE_HEADER_BYTES);
