...
public class ScriptOutputStrange extends ScriptOutput implements Serializable {
   ...
   public byte[] getAddressBytes() {
      return new byte[BitcoinILAddress.NUM_ADDRESS_BYTES];
   }

   @Override
   public BitcoinILAddress getAddress(NetworkParameters network) {
      // We cannot determine the address from scripts we do not understand
      return BitcoinILAddress.getNullAddress(network);
   }
}
