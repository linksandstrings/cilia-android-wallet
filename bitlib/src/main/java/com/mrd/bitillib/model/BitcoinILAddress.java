...
import com.mrd.bitillib.bitcoinj.Base58;
...
public class BitcoinILAddress implements Serializable, Comparable<BitcoinILAddress> {
   private static final long serialVersionUID = 1L;
   public static final int NUM_ADDRESS_BYTES = 21;

   private byte[] _bytes;
   private String _address;
   private BitcoinILSha256Hash scriptHash;
   private HdKeyPath bip32Path;

   public static BitcoinILAddress fromString(String address, NetworkParameters network) {
      BitcoinILAddress addr = BitcoinILAddress.fromString(address);
      if (addr == null) {
         return null;
      }
    ...
    *         null else
    */
   public static BitcoinILAddress fromString(String address) {
      if (address == null) {
         return null;
      }
      if (address.length() == 0) {
      ...
      return new BitcoinILAddress(bytes);
   }

   public static BitcoinILAddress fromP2SHBytes(byte[] bytes, NetworkParameters network) {
      if (bytes.length != 20) {
         return null;
      }
      ...
      return new BitcoinILAddress(all);
   }

   public static BitcoinILAddress fromStandardBytes(byte[] bytes, NetworkParameters network) {
      if (bytes.length != 20) {
         return null;
      }
      byte[] all = new byte[NUM_ADDRESS_BYTES];
   ...
   /**
    * Construct a BitcoinIL address from an array of bytes containing both the
    ...
    */
   public BitcoinILAddress(byte[] bytes) {
      _bytes = bytes;
      _address = null;
   }

   /**
    * Construct a BitcoinIL address from an array of bytes and the string
    ...
    *
    * @param bytes         containing the full address representation 1 + 20 bytes.
    * @param stringAddress the string representation of a BitcoinIL address
    */
   public BitcoinILAddress(byte[] bytes, String stringAddress) {
      _bytes = bytes;
   ...
   public String toString() {
      if (_address == null) {
         ...
         BitcoinILSha256Hash checkSum = HashUtils.doubleSha256(addressBytes, 0, NUM_ADDRESS_BYTES);
   ...
   public boolean equals(Object obj) {
      ...
      if (!(obj instanceof BitcoinILAddress)) {
         return false;
      }
      return BitUtils.areEqual(_bytes, ((BitcoinILAddress) obj)._bytes);
   }

   public static BitcoinILAddress getNullAddress(NetworkParameters network) {
      byte[] bytes = new byte[NUM_ADDRESS_BYTES];
      bytes[0] = (byte) (network.getStandardAddressHeader() & 0xFF);
      return new BitcoinILAddress(bytes);
   }

   public static BitcoinILAddress getNullAddress(NetworkParameters network, AddressType addressType) {
      byte[] bytes = new byte[NUM_ADDRESS_BYTES];
   ...
   @Override
   public int compareTo(@NotNull BitcoinILAddress other) {
   ...
   public BitcoinILSha256Hash getScriptHash() {
      if (scriptHash == null) {
         byte[] scriptBytes;
         if (isP2SH(getNetwork())) {
