...
abstract class AssetBitcoinILUriParser(open val network: NetworkParameters) : UriParser {
    protected fun parseParameters(uri: URI, coinType: CryptoCurrency): AssetUri? {
        ...
        if (addressString != null && Bip38.isBip38PrivateKey(addressString)) {
            ...
            if (coinType == BitcoinILMain.get() || coinType == BitcoinILTest.get()) {
                return PrivateKeyUri(addressString, label, "bitcoinil")
            }
    ...
    companion object {
        ...
        fun createUriByCoinType(coinType: CryptoCurrency,
            ...
            return when (coinType) {
                ...
                is BitcoinILMain, is BitcoinILTest -> BitcoinILUri(address, amount, label, paymentUri)
