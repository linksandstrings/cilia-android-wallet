 ...
 * retrieval next time they are requested.
 */
public class HDAccountKeyManagerIL {
   ...
   public static HDAccountKeyManagerIL createNew(HdKeyNode bip32Root, NetworkParameters network, int accountIndex,
   ...
   public static HDAccountKeyManagerIL createFromAccountRoot(HdKeyNode accountRoot, NetworkParameters network,
      ...
      return new HDAccountKeyManagerIL(accountIndex, network, secureKeyValueStore, derivationType);
   }

   protected HDAccountKeyManagerIL(SecureKeyValueStore secureKeyValueStore, BipDerivationType derivationType) {
      _secureKeyValueStore = secureKeyValueStore;
      this.derivationType = derivationType;
   }

   public HDAccountKeyManagerIL(int accountIndex, NetworkParameters network, SecureKeyValueStore secureKeyValueStore, BipDerivationType derivationType) {
   ...
   public InMemoryPrivateKey getBitcoinILPrivateKey(boolean isChangeChain, int index, KeyCipher cipher) throws KeyCipher.InvalidKeyCipher {
   ...
   public BitcoinILAddress getAddress(boolean isChangeChain, int index) {
      // See if we have it in the store
      ...
      BitcoinILAddress address = publicKey.toAddress(_network, derivationType.getAddressType());
   ...
   private static byte[] addressToBytes(BitcoinILAddress address) {
   ...
   private static BitcoinILAddress bytesToAddress(byte[] bytes, HdKeyPath path) {
      try {
