 ...
 */
class HDAccountContextIL @JvmOverloads constructor(
        val id: UUID,
        val accountIndex: Int,
        private var isArchived: Boolean,
        private var blockHeight: Int = 0,
        private var lastDiscovery: Long = 0,
        val indexesMap: MutableMap<BipDerivationType, AccountIndexesContextIL> = createNewIndexesContexts(BipDerivationType.values().asIterable()),
    ...
    constructor(context: HDAccountContextIL) : this(context.id, context.accountIndex,
     ...
     */
    fun persistIfNecessary(backing: Bip44BtcILAccountBacking) {
        if (isDirty) {
            persist(backing)
        }
    }
     ...
     */
    fun persist(backing: Bip44BtcILAccountBacking) {
    ...
    companion object {
        ...
        private fun createNewIndexesContexts(derivationTypes: Iterable<BipDerivationType>) =
                derivationTypes.map { it to AccountIndexesContextIL(-1, -1, 0) }
                        .toMap()
                        .toMutableMap()
    }
}

data class AccountIndexesContextIL(var lastExternalIndexWithActivity: Int, var lastInternalIndexWithActivity: Int,
