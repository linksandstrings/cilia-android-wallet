...
public abstract class ExchangeBasedCurrencyValue extends CurrencyValue {
   ...
   static public CurrencyValue from(BigDecimal value, String currency) {
      ...
      } else if (currency.equals(CurrencyValue.BTCIL)) {
         return new ExchangeBasedBitcoinILValue(currency, value);
      } else {
   ...
   static public CurrencyValue convertFromValue(ExchangeBasedCurrencyValue value, String targetCurrency, ExchangeRateProvider exchangeRateManager) {
      if (targetCurrency.equals(value.getCurrency())) {
         ...
         if (fx.getSourcePrice() == null || fx.getTargetPrice() == null) {
            ...
            } else if (targetCurrency.equals(CurrencyValue.BTCIL)) {
               return new ExchangeBasedBitcoinILValue(targetCurrency, (Long) null, value.getExactValue());
            } else {
         ...
         if (targetCurrency.equals(CurrencyValue.BTC)) {
            ...
            return new ExchangeBasedBitcoinILValue(targetCurrency, newValue, value.getExactValue());
         } else {
      ...
      if (targetCurrency.equals(exactValue.getCurrency())) {
         ...
         if (fx.getSourcePrice() == null || fx.getTargetPrice() == null) {
            if (targetCurrency.equals(CurrencyValue.BTC)) {
               ...
               return new ExchangeBasedBitcoinILValue(targetCurrency, (Long) null, exactValue);
            } else {
         ...
         } if (targetCurrency.equals(CurrencyValue.BTCIL)) {
            return new ExchangeBasedBitcoinILValue(targetCurrency, newValue, exactValue);
         } else {
   ...
   private static class GetExchangeRate {
      ...
      public GetExchangeRate invoke() {
         ...
         } else if (!sourceCurrency.equals(BTCIL)) {
            sourceExchangeRate = exchangeRateManager.getExchangeRate(targetCurrency, sourceCurrency);
            if (sourceExchangeRate != null && sourceExchangeRate.price != null) {
               sourcePrice = BigDecimal.valueOf(sourceExchangeRate.price);
            }
         ...
         } else if (!targetCurrency.equals(BTCIL)) {
            targetExchangeRate = exchangeRateManager.getExchangeRate(sourceCurrency, targetCurrency);
            if (targetExchangeRate != null && targetExchangeRate.price != null) {
               targetPrice = BigDecimal.valueOf(targetExchangeRate.price);
            }
