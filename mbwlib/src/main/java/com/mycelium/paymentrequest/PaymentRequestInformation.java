...
public class PaymentRequestInformation implements Serializable {
   ...
   public com.mrd.bitillib.model.OutputList getBitilOutputs() {
      return getBitilTransactionOutputs(this.paymentDetails);
   }
   ...
   private static com.mrd.bitillib.model.OutputList getBitilTransactionOutputs(PaymentDetails paymentDetails) {
      com.mrd.bitillib.model.OutputList ret = new com.mrd.bitillib.model.OutputList();
      for (Output out : paymentDetails.outputs) {
         ret.add(
                 Wire.get(out.amount, Output.DEFAULT_AMOUNT),
                 com.mrd.bitillib.model.ScriptOutput.fromScriptBytes(Wire.get(out.script, Output.DEFAULT_SCRIPT).toByteArray()));
      }
      return ret;
   }
   ...
   public Payment buildBitcoinILPaymentResponse(BitcoinILAddress refundAddress, String memo, BitcoinILTransaction signedTransaction) {
      byte[] scriptBytes = new ScriptOutputP2PKH(refundAddress.getTypeSpecificBytes()).getScriptBytes();
      Output refundOutput = new Output.Builder()
              .amount(getOutputs().getTotalAmount())
              .script(ByteString.of(scriptBytes))
